...
export const loginUser = createAsyncThunk(
  "auth/login",
  async (
    { formData, userType }: { formData: any; userType: UserType },
    { rejectWithValue }
  ) => {
    try {
      const endpoint =
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    updateUserProfile: (
      state,
      action: PayloadAction<{ doctor?: Doctor; patient?: Patient }>
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        loginUser.fulfilled,
        (state, action: PayloadAction<{ data: any; userType: UserType }>) => {
          state.loading = false;
