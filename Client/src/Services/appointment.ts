...
import type { ApiResponse, AppointmentRequestDto, AppointmentResponseDto, EmergencyAppointmentDto, PageResponse } from "../types/appointment"
...
export const getAppointmentDetails = async (id: string): Promise<AppointmentResponseDto> => {
  try {
    const response = await api.get<ApiResponse<AppointmentResponseDto>>(`${API_BASE_URL}/appointments/${id}`)
    return response.data.data
  } catch (error) {
