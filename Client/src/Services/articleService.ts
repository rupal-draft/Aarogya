...
export const deleteArticle = async (id: string): Promise<string> => {
  try {
...
export const commentArticle = async (
  comment: ArticleCommentRequestDTO
): Promise<void> => {
  try {
    await api.post(`/comment`, comment);
  } catch (error) {
    console.error(
      `Error commenting on article with ID ${comment.articleId}:`,
      error
    );
    throw error;
  }
...
export const getArticleLikesCount = async (id: string): Promise<number> => {
  try {
    ...
    console.error(
      `Error fetching like count for article with ID ${id}:`,
      error
    );
    throw error;
  }
